...
//                    $this->main_photo = $match[1];
                $this->main_photo = preg_replace('!(.+)\.(_V1_.+)\.jpg!ims', '$1._V1_QL75_UX280_CR0,15,280,280_.jpg', $match[1]);
            } else {
//                    $this->main_photo = str_replace('_SY140_SX100', '_SY600_SX400', $match[1]);
                $this->main_photo = $match[1];

            }
//            dump('thumb:'.$thumb.'-'.$this->main_photo);
        } else {
            return false;
        }
    ...
    public function savephoto($path, $thumb = true, $rerun = false)
    {
        ...
        if (strpos($req->getResponseHeader("Content-Type"), 'image/jpeg') === 0
          ...
          || strpos($req->getResponseHeader("Content-Type"), 'image/png') === 0
...
//                dump($block);
                if(count($matches[0]) <= 0){
                    preg_match_all('!<div class="soda.*?\s*<p>\s*(?<bio>.+?)\s</p>\s*\s*(?<author>.*?)\s*!ims',
                        $block[1], $matches);
